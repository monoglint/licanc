...
namespace {
    ...
    */
    manager::FileId register_file_dependency(manager::FileManager& file_manager, manager::FileId file_id, const std::string& absolute_file_path) {
        manager::FileManager::AddFileResult add_file_result = file_manager.add_file(absolute_file_path);
        
        if (add_file_result.has_value()) {
            manager::FileId new_file_id = add_file_result.value();
            ...
            manager::FileId found_file_id = file_manager.find_file(absolute_file_path).value();
    ...
    bool detect_file_interdependency(const manager::FileManager& file_manager, manager::FileId current_file_id, std::vector<manager::FileId>& file_stack, const std::string& absolute_file_path) {
        return std::find_if(file_stack.begin(), file_stack.end(), [&](manager::FileId frame_file_id) -> bool {
    ...
    */
    void handle_post_parse_file_imports(manager::CompilationEngine& engine, manager::FileId current_file_id, manager::CompilationFile& file, std::vector<manager::FileId>& file_stack) {
        ...
        for (frontend::ast::ImportDecl* import_decl_node : file.compiler_output_data.frontend.ast.import_nodes) {
            ...
            manager::FileId dependency_file_id = register_file_dependency(engine.file_manager, current_file_id, absolute_file_path);
    ...
    void parse_file(manager::CompilationEngine& engine, manager::FileId file_id, std::vector<manager::FileId>& file_stack) {
    ...
    void analyze_file(manager::CompilationEngine& engine, manager::FileId file_id) {
...
void manager::FileRefresher::recurse_mark_dirty(manager::FileId start) {
...
void manager::CompilationEngine::run_frontend(FileId target_file_id) {
    ...
    while (!file_stack.empty()) {
        manager::FileId file_id = file_stack.back();
        CompilationFile& file = file_manager.get_file(file_id).value();
